...
const data = [
    ...
    },
    {
        ...
        dn: null, valor: 3000.00, data_inicio: "2026-02-02"
    },
    {
    ...
    },
    {
        ...
        dn: null, valor: 3100.00, data_inicio: "2026-04-12"
    },
    {
    ...
    },
    {
        ...
        endereco: null, cep: null, municipio: null,
    ...
    },
    {
        ...
        endereco: "Tv. Henrique Dias, 577 - Jardim Aurora", cep: null, municipio: "Ourinhos (SP)",
...
data.forEach(item => {
    ...
    const dnVal = item.dn ? `'${item.dn}'` : "NULL";
    const addrVal = item.endereco ? `'${item.endereco.replace(/'/g, "''")}'` : "NULL";
    const cepVal = item.cep ? `'${item.cep}'` : "NULL";
    const cityVal = item.municipio ? `'${item.municipio.replace(/'/g, "''")}'` : "NULL";
    
    const sqlPatients = `INSERT INTO patients (id, nome, cpf, rg, data_nascimento, responsavel, resp_rg, resp_cpf, resp_endereco, resp_cidade, resp_cep, unidade, status) VALUES ('${patientId}', '${item.idoso.replace(/'/g, "''")}', '${item.cpf2}', '${item.rg2}', ${dnVal}, '${item.contratante.replace(/'/g, "''")}', '${item.rg1}', '${item.cpf1}', ${addrVal}, ${cityVal}, ${cepVal}, '${item.unidade}', 'ativo');`;
    sqlStatements.push(sqlPatients);
    
    // Entity registration
    const sqlEntities = `INSERT INTO entities (id, name, type, document) VALUES ('${entityId}', '${item.contratante.replace(/'/g, "''")}', 'customer', '${item.cpf1}');`;
    sqlStatements.push(sqlEntities);
    
    // Contract registration - Using DOUBLE QUOTES for case-sensitive camelCase columns
    const sqlContracts = `INSERT INTO contracts ("pacienteId", "pacienteNome", valor, "dataInicio", status) VALUES ('${patientId}', '${item.idoso.replace(/'/g, "''")}', ${item.valor}, '${item.data_inicio}', 'ativo');`;
    sqlStatements.push(sqlContracts);
});

fs.writeFileSync('insert_patients.sql', sqlStatements.join('\n'));
